...
PATCH="$OVRDIR/libandroid_runtime.nop-fd-reopen-fail-1.so"
...
od -An -tx1 -j $((0x1d40f0)) -N 96 "$PATCH"

echo
echo "--- bytes before ---"
echo -n "FD ReopenOrDetach fail call @ 0x1d4124: "
BYTES="$(od -An -tx1 -j "$OFF_FAIL" -N 4 "$PATCH" | tr -d ' \n')"
echo "$BYTES"

if [ "$BYTES" != "00013fd6" ]; then
  echo "WARN: expected blr x8 bytes 00 01 3f d6 at 0x1d4124"
  echo "Refusing to patch. Run:"
  echo "aarch64-linux-gnu-objdump -d -C --start-address=0x1d40e0 --stop-address=0x1d4160 zygote-symbols/libandroid_runtime.so.real"
  exit 1
fi

echo
echo "--- patch: FD ReopenOrDetach fail call @ 0x1d4124 -> nop ---"
...
echo
echo "LIBANDROID_RUNTIME_NOP_FD_REOPEN_FAIL_1_OK"
