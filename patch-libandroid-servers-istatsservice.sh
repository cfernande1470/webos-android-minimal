...
OVR=$SIDE/overrides-libandroid-servers-istats
WORK=work-libandroid-servers-istats
...
echo "--- pull current libandroid_servers.so (keeps existing PowerStats patch if bind-mounted) ---"
...
echo
echo "--- disasm before around startIStatsService entry estimate 0x74c98 ---"
aarch64-linux-gnu-objdump -d \
  --start-address=0x74c70 \
  --stop-address=0x74d50 \
...
print(f"PATCHED startIStatsService entry vaddr=0x{vaddr:x} file_off=0x{off:x}")
print(f"old={old.hex()}")
print(f"new={new.hex()}")
PY
...
echo "--- bind patched libandroid_servers with PowerStats + IStats bypasses ---"
...
echo "PATCH_LIBANDROID_SERVERS_ISTATS_BOUND"
EOF2

echo
echo "PATCH_LIBANDROID_SERVERS_ISTATS_DONE"
