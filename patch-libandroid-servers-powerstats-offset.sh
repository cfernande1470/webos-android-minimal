...
WORK=work-libandroid-servers-powerstats-offset
...
echo "--- pull current libandroid_servers.so ---"
...
echo
echo "--- disasm before around PowerStats nativeInit entry 0x6ef00 ---"
aarch64-linux-gnu-objdump -d \
  --start-address=0x6eee0 \
  --stop-address=0x6ef80 \
  "$WORK/libandroid_servers.so.patched" || true

echo
echo "--- disasm before around connectToPowerStatsHal area ---"
aarch64-linux-gnu-objdump -d \
...
print(f"PATCHED PowerStats nativeInit entry vaddr=0x{vaddr:x} file_off=0x{off:x}")
print(f"old={old.hex()}")
print(f"new={new.hex()}")
PY
...
echo "PATCH_LIBANDROID_SERVERS_POWERSTATS_OFFSET_BOUND"
EOF2

echo
echo "PATCH_LIBANDROID_SERVERS_POWERSTATS_OFFSET_DONE"
