...
ls -l "$SIDE/bin/property_service_ack_shim" "$SIDE/property_service_ack_shim" "$SIDE/zygote_socket_wrap" 2>/dev/null || true

SHIM="$SIDE/bin/property_service_ack_shim"
[ -x "$SHIM" ] || SHIM="$SIDE/property_service_ack_shim"

if [ ! -x "$SHIM" ]; then
  echo "ERROR: no encuentro property_service_ack_shim ejecutable"
  exit 1
fi
...
echo
echo "--- arrancar ack shim ---"
nohup "$SHIM" "$ROOTFS/dev/socket/property_service" \
  </dev/null >"$LOGDIR/property_service_ack_shim.fixed2.log" 2>&1 &

sleep 1
ls -l "$ROOTFS/dev/socket/property_service" || {
  echo "ERROR: el shim no creó property_service"
  cat "$LOGDIR/property_service_ack_shim.fixed2.log" 2>/dev/null || true
  exit 1
}
...
chroot "$ROOTFS" /system/bin/init second_stage \
  >"$LOGDIR/init.second_stage.fixed2.log" 2>&1 &
...
ls -l "$ROOTFS/system/bin/app_process64" "$ROOTFS/apex/com.android.runtime/bin/linker64" 2>/dev/null || true
...
echo
echo "--- shim log ---"
tail -n 80 "$LOGDIR/property_service_ack_shim.fixed2.log" 2>/dev/null || true

echo
echo "--- init log tail ---"
tail -n 120 "$LOGDIR/init.second_stage.fixed2.log" 2>/dev/null || true
