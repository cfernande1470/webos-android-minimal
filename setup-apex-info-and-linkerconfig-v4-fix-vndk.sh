...
echo
echo "--- fix missing product/system_ext/vndk.v33 before linkerconfig ---"

mkdir -p "$ROOTFS/product" "$ROOTFS/system_ext" "$ROOTFS/odm" "$ROOTFS/vendor" 2>/dev/null || true

# Android 13/linkerconfig suele buscar com.android.vndk.v33.
# En este rootfs tenemos com.android.vndk.current, así que creamos alias.
...
if [ -e "$ROOTFS/system/apex/com.android.vndk.current" ] && [ ! -e "$ROOTFS/system/apex/com.android.vndk.v33" ]; then
  ln -s com.android.vndk.current "$ROOTFS/system/apex/com.android.vndk.v33" 2>/dev/null || true
fi

echo "--- vndk alias check ---"
ls -ld "$ROOTFS/apex/com.android.vndk.current" "$ROOTFS/apex/com.android.vndk.v33" 2>&1 || true
ls -ld "$ROOTFS/product" "$ROOTFS/system_ext" "$ROOTFS/vendor" "$ROOTFS/odm" 2>&1 || true
