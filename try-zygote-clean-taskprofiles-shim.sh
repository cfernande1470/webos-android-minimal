...
echo "--- sanity clean taskprofiles shim ---"
...
echo
echo "--- start zygote clean + taskprofiles shim ---"
...
nohup env -i \
  ...
  "$WRAP" \
    ...
    /system/bin/app_process64 \
      ...
      --socket-name=zygote \
      --abi-list=arm64-v8a \
  >"$LOGDIR/zygote64.clean-taskprofiles-shim.log" 2>&1 &
...
echo
echo "--- log ---"
tail -n 260 "$LOGDIR/zygote64.clean-taskprofiles-shim.log"
...
echo
echo "--- maps shim/art check ---"
if [ -n "$PID" ]; then
  grep -E 'libzygote_taskprofiles_shim|libprocessgroup|libart\.so|libopenjdkjvm\.so' /proc/$PID/maps || true
fi
